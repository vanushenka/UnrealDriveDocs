# Road Editor Modes
Spline Editor Modes это режимы редактирования **URoadSplineComponent**. Каждый из режимов предоставят доступ к редактированиях соответствующих свойств, описанных в [Road Model](RoadModel.md).
![alt text](img/edit-modes2.png).  
Есть **Base** и **Attributes** режимы.

## Spline Mode
Этот режим позволяет взаимодеиствовать с **URoadSplineComponent** как с обычным сплайном UnrealEngine - создавать и удалять узлы, редакторов тангенты. Сам сплайн, в этом режиме, имеет розовый цвет.  
...
Центральная сиреневая линия - это [Center Lane](RoadModel.md#road-lanes) и на най располагаются узлы, потянув за которые, можно изменить длину секции:  
...
## Offset Mode
Этот режим позволят задавать смещения **Center Lane** относительно [Road Reference Line](RoadModel.md#road-reference-line) и соответствует [Lane Offset](RoadModel.md#lane-offset) описанный в [Road Model](RoadModel.md).  
