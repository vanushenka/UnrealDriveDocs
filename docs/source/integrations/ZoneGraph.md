...
![The Build ZoneGraph item in the Build menu](/img/zone-graph-build.png)  

It is convenient to view the visualization of the constructed `ZoneGraphData` by activating the **Navigation** flag:  
![The generated ZoneGraph lanes visualized with the Navigation debug flag](/img/zone-graph-view-debug.png)  
...
![The URoadGraphDataComponent required for ZoneGraph generation on a road actor](/img/zone-graph-data.png)  
...
![The URoadGraphDataComponent data populated by a bake](/img/zone-graph-data2.png)  
...
![Road Lane Types and Zone Graph Tags settings in MetaRoad Editor project settings](/img/zone-graph-settings.png)  

The example below shows that a **ZoneGraph** section with **Tag** = Tram was generated for the tram lane:  
![A ZoneGraph section tagged Tram generated for the tram lane](/img/zone-graph-settings2.png)  

## See also

- [Baking](/baking/BakeStaticMesh.md) — generates the `URoadGraphDataComponent` that ZoneGraph reads.
- [Landscape](/integrations/Landscape.md) — another Pro integration driven from road spline data.
