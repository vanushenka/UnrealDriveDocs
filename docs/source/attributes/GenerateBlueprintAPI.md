# Generate Attributes — Blueprint API

Reference for building your **own** Generate attribute in Blueprint. `URoadLaneAttributeGenerateDescriptor`
(display name **Generate**) is the base class of every [Generate attribute](/attributes/GenerateAttributes.md); it
generates nothing by itself. The mesh pipeline reads the lane's keys, walks the anchor path, splits it into
segments, and calls your Blueprint events so you can emit whatever geometry you want — no C++ required.
...
1. **Create the asset** — an [Attribute Profile](/profiles/AttributeProfile.md) with **RoadLaneAttributeGenerateDescriptor**
   (**Generate**) as the parent class, as shown in [Create new Attribute](/attributes/GenerateAttributes.md#create-new-attribute) above.
...
- [Generate Attributes](/attributes/GenerateAttributes.md) — the attribute family and how it follows the lane.
- [Attribute Profile](/profiles/AttributeProfile.md) — creating the asset this Blueprint lives in.
- [Attribute Mode](/editing/AttributeMode.md) — placing keys once your type exists.
