...
```{note}
**New in 3.2.0.** A profile's shapes can carry [holes](/profiles/PolygonProfile.md#holes) — inner contours cut out
of them — and this attribute carries them into the generated mesh. Placed on a lane, the shape shows the **road
surface** through its holes, since the lane already owns that area.
```
