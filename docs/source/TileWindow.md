...
## Система координат
Достаточно задать ```Longitude``` и ```Latitude``` **Tile Window** мгновенно перегенерит область окна:  
![alt text](img/tile-latlon.gif)  

Параметр ```Use World Coordinate Space``` определяет начала координат для ```Longitude``` и ```Latitude``` в мире Unreal Engine:
  - Если флаг не установлен, то за начала координат берутся координаты **Tile Window** Актора
  - Если флаг установлен, то за начало координат берется точка (0, 0)  мира Unreal Engine. В этом случае при изменении координат **Tile Window** Актора, будет автоматически пересчитана область окна **Tile Window** согласно новым координатам. Обычно такой режим удобнее использовать для воссоздании больших участков территорий земли. Так как в этом случа  **Tile Window** работает как "окно" в реальный мир, и при его перемещении будет автоматически отображен новый участок карты:  
